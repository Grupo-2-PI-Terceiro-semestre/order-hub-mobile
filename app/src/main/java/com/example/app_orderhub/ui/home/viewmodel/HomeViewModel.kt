package com.example.app_orderhub.ui.home.viewmodel
...
class HomeViewModel : ViewModel() {
    ...
    private val homeRepository = AppModule.homeRepository
    ...
    fun getEnterprises(onSuccess: (List<Enterprise>) -> Unit, onError: (String) -> Unit) {
        viewModelScope.launch {
            ...
            try {
                val response = homeRepository.getEnterprise(_param.value ?: "")
    ...
    fun getEnterprisesForCategory(onSuccess: (List<Enterprise>) -> Unit, onError: (String) -> Unit) {
        viewModelScope.launch {
            _isLoadingCategory.value = true
            _errorMessage.value = null

            try {
                val response = homeRepository.getEnterpriseForEnterprise(_categoryParam.value ?: "")
                val enterprises = response.toEnterprises()
                _enterprisesForCategory.value = enterprises
                onSuccess(enterprises)
            } catch (e: Exception) {
                _errorMessage.value = e.message
                onError(e.message ?: "Erro desconhecido")
